...
- **Tailwind CSS v4 + shadcn/ui** (Radix primitives + `class-variance-authority`). Global styles and theme tokens in `src/app/globals.css`. The `cn()` class-merge helper lives in `src/lib/utils.ts`.
