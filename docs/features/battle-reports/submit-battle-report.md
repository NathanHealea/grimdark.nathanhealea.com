...
- `BattleReport` — full database row type
...
`submitBattleReport()` — authenticates user, checks member/admin role, validates all fields, validates attacker != defender, inserts into database, revalidates paths.
...
- Three sections: Attacker, Defender, Game Details
...
- **Defender Score:** The ending score of the defending player.
...
- **Rounds:** The number of rounds played (1-5).
