  ...
  const isUserSelf = async (userId: number): Promise<boolean> => {
    ...
    try {
      ...
      if (error instanceof Error) {
        throw new Error(`Error checking if user is self: ${error.message}`)
      } else if (typeof error === 'string') {
        throw new Error(`Error checking if user is self: ${error}`)
      } else {
        throw new Error('An unknown error occurred while checking if user is self.')
      }
    }
