...
export default function EditProfileForm(props: EditProfileFormProps) {
  ...
  if(state.success) {
    redirect('/profile?status=success&message=Profile updated successfully.')
  }
  ...
  return (
    <form className="form flex flex-col gap-4" action={formAction}>
      ...
      <fieldset className="fieldset bg-base-200  rounded-bo border border-warning rounded-lg p-4 w-full gap-4">
        ...
        {/* Password - Current */}
        <div className="flex flex-col gap-1 w-full">
          <label htmlFor="password_current" className={`label ${state.errors?.passwordCurrent ? 'text-error' : ''}`}>
            <span className="label-text">Current Password</span>
          </label>
          <label className={`input w-full ${state.errors?.passwordCurrent ? 'input-error' : 'validator'}`}>
            <svg className="h-[1em] opacity-50" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <g strokeLinejoin="round" strokeLinecap="round" strokeWidth="2.5" fill="none" stroke="currentColor">
            ...
            <input id="password_current" name="password_current" type="password" placeholder="Current Password" />
          </label>

          {state.errors?.passwordCurrent && (
            <ul className="list text-error">
              {state.errors.passwordCurrent.map((error, index) => (
                <li className="list-row" key={index}>
                  {error}
                </li>
              ))}
