...
BEGIN
    ...
    IF existing_user_id IS NOT NULL THEN
        ...
        SET
          ...
          profile_url = COALESCE(NEW.raw_user_meta_data->>'picture', NEW.raw_user_meta_data->>'avatar_url', public.users.profile_url),
...
-- This function synchronizes user data from auth.users to public.users.
-- It creates a new user record in public.users if one doesn't exist,
-- or updates an existing one with the latest information.
CREATE OR REPLACE FUNCTION public.handle_user_update()
RETURNS TRIGGER AS $$
BEGIN
  -- Check if a user with the NEW.id (from auth.users) already exists in public.users
