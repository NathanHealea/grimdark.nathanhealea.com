...
create table
  public.missions (id serial primary key, name text unique not null);

insert into
  public.missions (name)
values
  ('Burden of Trust'),
  ('Hidden Supplies'),
  ('Linchpin'),
...
create table
  public.deployments (id serial primary key, name text unique not null);

insert into
  public.deployments (name)
values
  ('Dawn of War'),
  ('Hammer and Anvil'),
  ('Search and Destroy'),
...
insert into
  public.battle_points (name, size)
values
  ('Combat Patrol', 500),
  ('Blitz', 750),
  ('Incursion', 1000),
...
create trigger on_battle_reports_updated before
update on public.battle_reports for each row execute function public.handle_updated_at ();
...
create policy "Missions are publicly readable" on public.missions for
select
  to anon,
  authenticated using (true);

create policy "Deployments are publicly readable" on public.deployments for
select
  to anon,
  authenticated using (true);

create policy "Battle points are publicly readable" on public.battle_points for
select
  to anon,
  authenticated using (true);

-- Battle reports: publicly readable
create policy "Battle reports are publicly readable" on public.battle_reports for
select
  to anon,
  authenticated using (true);

-- Battle reports: members and admins can insert
create policy "Members and admins can submit battle reports" on public.battle_reports for insert to authenticated
with
  check (
...
create policy "Reporter or admin can update battle reports" on public.battle_reports for
